...
class CartProvider extends ChangeNotifier {
  ...
  void addToCart(ProductModel product) {
    _cartBox.add(product); // Add product to Hive box
    notifyListeners();
  }

  void removeFromCart(ProductModel product) {
    final key = _cartBox.keyAt(_cartBox.values.toList().indexOf(product));
    _cartBox.delete(key); // Remove product from Hive box
    notifyListeners();
  }
}
...
class FavoriteProvider extends ChangeNotifier {
  ...
  void addToFavorite(ProductModel product) {
    _favoriteBox.add(product); // Add product to Hive box
    notifyListeners();
  }
  ...
  void removeFromFavorite(ProductModel product) {
    ...
    _favoriteBox.delete(key); // Remove product from Hive box
    notifyListeners();
  }
}
